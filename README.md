...
- **Agent runtime integration** — `ideon agent install <runtime>` configures skills and MCP registration for supported hosts (`pi`, `claude`, `cursor`, `codex`, `gemini`, `vscode`, `opencode`, `hermes`, `chatgpt`, `claude-desktop`, `generic-mcp`). Default installs the `ideon-cli` skill; use `--mcp-skill` for MCP + `ideon-mcp`. Check artifacts with `ideon agent status --json`.
