...
## Rule: Assign targetKeywords Per Section
Description: During planning, assign 0–2 target keywords per section based on semantic fit. Every keyword in the plan must appear in the title, a section heading, or a section's targetKeywords list.
Negative example: Passing the full keyword list to every section writer with no assignment strategy.
Positive example: Section "How Pod Scheduling Works" gets targetKeywords ["pod scheduling", "Kubernetes"]; section "Cost Trade-offs" gets ["infrastructure costs"].

## Rule: Use Secondary Keywords in H2 Headings When Natural
Description: At least one secondary keyword should appear in a major H2 section heading when it fits the section topic. Prefer readable headings over exact-match forcing.
Negative example: "Best SEO SEO Tools SEO Software SEO Guide" as a section title.
Positive example: "How to Evaluate SEO Tools for Content Teams" when "SEO tools" is a secondary keyword.

## Rule: Weave Keywords Into Body Prose Naturally
Description: Section writers should use assigned targetKeywords in body prose where they fit the section topic. Use natural variants and synonyms when exact match would sound awkward.
Negative example: Repeating the same keyword phrase five times in one paragraph.
Positive example: Using "content strategy" once explicitly and "editorial planning" as a variant in the same section.

## Rule: Do Not Duplicate Keywords as Standalone Headings
...
Negative example: Three planned keywords with one never appearing anywhere in the draft.
Positive example: Keyword coverage map checked: primary in title+intro; each secondary in assigned section heading or body.
