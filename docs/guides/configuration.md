...
- `TELEPAT_OPENROUTER_KEY` and `TELEPAT_REPLICATE_TOKEN` from environment variables override keychain-stored secrets.
...
- Set `TELEPAT_DISABLE_KEYTAR=true` to skip keychain access entirely in container or CI environments.
