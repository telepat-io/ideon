...
| `--author <slug>` | 否 | string | 本系列文章的默认作者 slug。 |
| `--experience <text>` | 否 | string | 本系列的常驻轶事或第一手经验（运行时可由 `--experience` 补充）。 |
...
| `--author <slug>` | 否 | string | 新的默认作者 slug。 |
| `--experience <text>` | 否 | string | 新的系列常驻经验备注。 |
| `--unset-author` | 否 | boolean | 移除默认作者关联。 |
