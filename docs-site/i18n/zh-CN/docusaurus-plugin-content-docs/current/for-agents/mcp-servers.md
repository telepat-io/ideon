...
SEO 检查作为流水线第 4 阶段在 `ideon_write` / `ideon_write_resume` 中运行——没有独立的 SEO MCP 工具。`ideon_write` 可选参数：`noSeoCheck`、`seoCheckMode`（`errors-only` | `strict`）、`seoCheckMaxTurns`（1–20）；`ideon_write_resume` 可选参数：`seoCheck`（强制重跑）、`seoCheckMode`、`seoCheckMaxTurns`。结果写入 `meta.json`（`seoCheck`）。
